<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//div">
        <xsl:choose>
            ...
            <xsl:when test="@type='poem' and not(descendant::del[not(@ana)])"/>
            <!--division: poem (emended) -->
            <!--division: poem (not emended) -->
            <xsl:when test="@type='poem' and descendant::del/note"/>
            <!--division: poem (emended) -->
            <xsl:when
                test="@type='poem' and descendant::head/del[not(@ana)] or descendant::title/del[not(@ana)]or descendant::del[not(@ana)]/title or descendant::l/del[not(@ana)]">
                <table border="0" cellpadding="0" cellspacing="0">
                    <tr>
                        <td>&#160;</td>
                    </tr>
                ...
                <table border="0" cellpadding="0" cellspacing="0">
                    <tr>
                        <td valign="top" width="100">
                            <!-- title in process -->
                            <xsl:if
                                test=" not(@select='final') and not(descendant::title[@type='subtitle'])">
                                <span class="revision">title(<xsl:value-of select="@ana"/>)</span>
                            </xsl:if>
                            <!-- subtitle  in process-->
                            <xsl:if
                                test="not(@select='final') and descendant::title[@type='subtitle']">
                                <span class="revision">subtitle(<xsl:value-of select="@ana"
                                    />)</span>
                            </xsl:if>
                            <!-- title revised -->
                            <xsl:if
                                test="@select='final' and not(descendant::title[@type='subtitle'])"
                                    >title<span class="revision">(<xsl:value-of select="@ana"
                                    />)</span></xsl:if>
                        </td>
                        <!-- subtitle revised -->
                        <xsl:if test="@select='final' and descendant::title[@type='subtitle']"
                                >subtitle<span class="revision">(<xsl:value-of select="@ana"
                                />)</span></xsl:if>
                        <td valign="top">
                            <xsl:apply-templates/>
                        </td>
            ...
            <tr>
                ...
                <td valign="top">
                    <xsl:choose>
                        ...
                        <!-- line not flush left-->
                        <xsl:when test="@ana and @rend and descendant::text()">
                            <span class="layout">[<xsl:value-of select="@rend"/>]</span>
                            <xsl:apply-templates/>
                        </xsl:when>
                        <!--line spacing in process of revision (initial) -->
                        <xsl:when
                            test="@ana and @rend and @ana and not(@select) and not(descendant::add[not(@ana)]) and not(descendant::text()[normalize-space()])">
